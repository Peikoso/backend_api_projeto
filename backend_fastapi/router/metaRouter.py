...
async def get_metas(db: AsyncSession = Depends(get_session)):
    query = text('SELECT * FROM meta')
...
async def get_meta_by_id(id_meta: int, db: AsyncSession = Depends(get_session)):
    query = text('SELECT * FROM meta WHERE id_meta = :id_meta')
    result = await db.execute(query.bindparams(id_meta=id_meta))
...
async def create_meta(meta: MetaCreate, db: AsyncSession = Depends(get_session), current_user=Depends(get_current_user)):
    query = text(
        """
        INSERT INTO meta (descri, data_inicio, data_fim, valor, id_user)
        VALUES (:descri, :data_inicio, :data_fim, :valor, :id_user)
        RETURNING id_meta;
        """
    )

    query = query.bindparams(descri=meta.descri, data_inicio=meta.data_inicio, data_fim=meta.data_fim, valor=meta.valor, id_user=current_user.id_user)
    result = await db.execute(query)
...
async def update_meta(id_meta: int, meta: MetaCreate, db: AsyncSession = Depends(get_session), current_user=Depends(get_current_user)):
    query = text(
        """
        UPDATE meta
        SET descri = :descri, data_inicio = :data_inicio, valor = :valor, data_fim = :data_fim
    ...
    result = await db.execute(
        query.bindparams(
            descri=meta.descri, data_inicio=meta.data_inicio, data_fim=meta.data_fim, valor=meta.valor, id_user=current_user.id_user, id_meta=id_meta
        )
    )
