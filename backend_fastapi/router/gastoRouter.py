...
from backend_fastapi.schema.gastoMensalSchema import GastoMensalResponse, GastoMensalComparativo
...
async def get_gasto_mensal_categoria(
...
async def get_gasto_mensal(
    ano: int, mes: int, db: AsyncSession = Depends(get_session), current_user = Depends(get_current_user)
):
    query = text(
        """
        SELECT ano, mes, categoria_despesa, total_gasto, percentual
...
async def get_gasto_comparativo_mensal(
    ano: int, mes: int, db: AsyncSession = Depends(get_session), current_user = Depends(get_current_user)
):
    query = text(
        """
