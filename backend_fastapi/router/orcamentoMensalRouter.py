...
async def get_orcamentoMensal(ano: int = Query(..., ge=2025, le=2100, description="Ano desejado"),
...
async def create_orcamentoMensal(orcamentoMensal: OrcamentoCreate, db: AsyncSession = Depends(get_session), current_user = Depends(get_current_user)):
    try:
        ...
        query = query.bindparams(mes=orcamentoMensal.mes,
        ...
        if 'unique_orcamento_usuario_mes_categoria' in str(e.orig):
            raise HTTPException(
                ...
                detail="Já existe um orçamento mensal para este mês, ano e categoria."
            )
        else:
...
async def update_orcamentoMensal(id_orcamento:int, orcamentoMensal: OrcamentoCreate, db: AsyncSession = Depends(get_session), current_user = Depends(get_current_user)):
    try:
        query = text(
        """
